...
export const db = {
  ...
  // Ensures exactly the 4 core categories and 20 basic food products are present.
  // Users can add more categories and items themselves.
  async seedNewUserInventory() {
    if (usePostgres) {
      ...
      for (const item of initialItems) {
        const exists = fileStore.data.items?.find(i => i.id === item.id);
        if (!exists) {
          fileStore.data.items = fileStore.data.items || [];
          fileStore.data.items.push({ ...item, last_updated: new Date().toISOString() });
        }
      }
      fileStore.save();
    }
